...
- (LEEConfigToStringAndColorAndBlock)LeeCustomAction{
    
    return ^(NSString *title,UIColor *color , void(^block)(void)){
...
- (LEEConfigToConfigTextView)LeeAddTextView{
    
    return ^(void (^block)(UITextView *)){
        
        return self.LeeAddItem(^(LEEItem *item) {
            
            item.type = LEEItemTypeTextView;
...
@interface LEEItemTextView : UITextView

@property (nonatomic , strong ) LEEItem *item;

+ (LEEItemTextView *)textView;

@end

@implementation LEEItemTextView

+ (LEEItemTextView *)textView{
    
    return [[LEEItemTextView alloc] init];
}
    ...
    [self.config.modelItemArray enumerateObjectsUsingBlock:^(id _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        switch (item.type) {
            ...
            case LEEItemTypeTextField:
            {
                ...
                break;
            case LEEItemTypeTextView:
            {
                LEEItemTextView *textView = [LEEItemTextView textView];
                
                textView.frame = CGRectMake(0, 0, 0, 40.0f);
                
                textView.layer.cornerRadius = 2;
                
                textView.layer.borderColor = [UIColor colorWithRed:229/255.0 green:229/255.0 blue:229/255.0 alpha:1].CGColor;
                ...
                [self.alertItemArray addObject:textView];
                
                void(^block)(UITextView *textView) = item.block;
